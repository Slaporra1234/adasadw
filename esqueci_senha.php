...
if (isset($_POST['submit_email'])) {
    ...
    if ($user) {
        ...
        $reset_link = "http://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . "/reset_senha.php?token=$token";
        
        $subject = "Recuperação de Senha - ProMarket Brasil";
        $message = "Olá " . htmlspecialchars($user['nome']) . ",\n\n";
        $message .= "Você solicitou a recuperação de senha para sua conta no ProMarket Brasil.\n";
        $message .= "Clique no link abaixo para redefinir sua senha (válido por 1 hora):\n\n";
        $message .= $reset_link . "\n\n";
        $message .= "Se você não solicitou esta alteração, ignore este email.\n\n";
        $message .= "Atenciosamente,\nEquipe ProMarket Brasil";
        
        // Enviar email usando PHPMailer (mais robusto que mail() nativo)
        ...
        try {
            ...
            $msg = 'Um email com instruções para redefinir sua senha foi enviado para ' . htmlspecialchars($email);
            $msg_type = 'success';
        } catch (Exception $e) {
...
<head>
    <meta charset="UTF-8">
    <title>Recuperar Senha - MarketPlace Brasil</title>
...
<body>
    <div class="main-card">
        ...
        <form method="post">
            ...
            <button type="submit" name="submit_email" class="auth-btn">Enviar Link</button>
        </form>
        <div class="switch">
            Lembrou sua senha? <a href="login_cadastro.php">Faça login</a>
        </div>
    </div>
</body>
</html>